  ...
  $(Screw)
    .bind('loaded', function() {    
      $('div.describe>h3, .describe>h1, .it>h2')
        .click(function() {
          document.location.search = '?screwunitSubset=' + $(this).parent().fn('selector') + '&' + location.search.replace(/screwunitSubset\=[^&]*/, "").replace(/^\?/, "").replace(/\&$/, "");
          return false;
        })
	  $('div.describe>h3')
    ...
    .bind('after', function() {
      ...
      $('div.describe>h3, .describe>h1, .it>h2')
        .each(function() {
          var text = $(this).text();
            var uri = location.href.split('?')[0] + '?screwunitSubset=' + $(this).parent().fn('selector') + '&' + location.search.replace(/screwunitSubset\=[^&]*/, "").replace(/^\?/, "").replace(/\&$/, "");
